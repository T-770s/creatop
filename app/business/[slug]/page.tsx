...
export default function BusinessPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  if (!business) {
    return (
      <div className="w-screen h-screen bg-[#030303] flex flex-col items-center justify-center gap-6">
        ...
        <button onClick={() => router.push('/world')} className="text-indigo-400 text-sm hover:text-indigo-300 transition-colors" data-hover>
          ← חזור לעולם
        </button>
      </div>
    );
  ...
  return (
    <main className="relative w-screen h-screen overflow-hidden">
        ...
        <div className="max-w-2xl">
          ...
          <div className="flex gap-2 mt-4 flex-wrap">
            {business.tags.map(tag => (
              <span
                key={tag}
                className="px-3 py-1 rounded-full text-xs glass-bright"
                style={{ borderColor: business.colors.glow + '40', color: business.colors.glow }}
              >
                {tag}
              </span>
            ))}
          ...
          <div className="flex gap-3 mt-6">
            <button
              className="px-6 py-3 rounded-xl text-sm font-semibold text-white transition-all hover:scale-105 hud-border"
              style={{ background: business.colors.primary + '30' }}
              data-hover
            >
              כנס לעסק
            </button>
            <button
              className="px-6 py-3 rounded-xl text-sm font-medium text-white/50 glass hover:text-white/80 transition-all"
              data-hover
            >
              שמור למועדפים
            </button>
      ...
      {/* Color accent line */}
      <div
        className="absolute bottom-0 inset-x-0 z-30 h-[2px] pointer-events-none"
        style={{ background: `linear-gradient(90deg, transparent, ${business.colors.glow}, transparent)` }}
      />
    </main>
  );
}
